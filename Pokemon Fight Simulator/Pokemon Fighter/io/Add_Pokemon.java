...
public abstract class Add_Pokemon {
	...
	public static void generate(String name, Types type, Types type2, int hp, int atk, int def, int spatk, int spdef,
			int speed, String evolve) {
		...
		if (type2 != null) {
			System.out.println("\nCreating new Pokemon " + name + " of type " + type.toString() + "-" + type2.toString()
					+ " evolving from " + evolve + " with stats:\nHPs: " + hp + "\nAtk: " + atk + "\nDef: " + def
					+ "\nSpAtk: " + spatk + "\nSpDef: " + spdef + "\nSpeed: " + speed);
		} else {
			System.out.println("\nCreating new Pokemon " + name + " of type " + type.toString() + " evolving from "
					+ evolve + " with stats:\nHPs: " + hp + "\nAtk: " + atk + "\nDef: " + def + "\nSpAtk: " + spatk
					+ "\nSpDef: " + spdef + "\nSpeed: " + speed);
		}
				...
				try {
					...
					if (evolve.equals("Pokemon")) {
						...
						if (!evolveType.equals(type)) {
							writer.write(
									"import pokemons." + evolveType.toString().toLowerCase() + "." + evolve + ";\r\n");
						}
					}
		...
		if (operationSuccess) {
			System.out.println("New pokemon file created at '" + file.getAbsolutePath() + "' ! ("
					+ (System.currentTimeMillis() - timer) + "ms)\nYou may need to refresh the " + type.toString()
					+ " package");
		} else {
